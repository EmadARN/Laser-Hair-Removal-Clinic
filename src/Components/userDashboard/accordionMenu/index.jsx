          ...
          <Stack>
            {reserveInformation.reserve.map((item, index) => (
              ...
              >
                ...
                <Box gap={2} display={"flex"} as="span">
                  <p> مدت زمان جلسه:</p>
                  {item.request_time_int}
                </Box>
                <Box gap={2} display={"flex"} as="span">
                  {" "}
                  <p>نواحی انتخاب شده:</p>
                  {item.laser_area_name}
                </Box>
                <Box gap={2} display={"flex"} as="span">
                  <p>ودیعه جهت رزرو نوبت:</p> {item.total_payment_amount}
                </Box>

                <Box gap={2} display={"flex"} as="span">
                  <p> مبلغ کل:</p> {item.total_price_amount}
                </Box>
              </Box>
            ))}
          </Stack>
